...
function prev()
{
    ...
    for(let i = sources.length - 1; i >= 0; i--)
    {
        ...
        {
            //  Update variables
            alt = i + 1 > 2 ? sources[i-1].alt : sources[sources.length -1].alt;
            caption = i + 1 > 2 ? sources[i-1].caption : sources[sources.length -1].caption;
            source = i - 1 >= 0 ? path + sources[i-1].src : path + sources[sources.length -1].src;
        }
    }
...
{
    //  Calculated readtime
    // https://www.w3schools.com/howto/howto_js_typewriter.asp || TypeEffect
